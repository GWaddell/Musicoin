...
#if defined(NDEBUG)
# error "Musicoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Musicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Musicoin Signed Message:\n";
...
{
    // Musicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    if(nHeight < 175316)
    {
        nSubsidy = 5 * COIN;  //First year limited release
    }
    else
    {
        nSubsidy = 10 * COIN;  //After first year, release without restriction
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Musicoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Musicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Musicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Musicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Musicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Musicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MusicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MusicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MusicoinMiner(CWallet *pwallet)
{
    printf("MusicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("musicoin-miner");
    ...
    try { loop {
        ...
        printf("Running MusicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MusicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MusicoinMiner, pwallet));
}
